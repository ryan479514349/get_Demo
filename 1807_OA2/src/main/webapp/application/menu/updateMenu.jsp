...
<body>
<div class="pd-20">
	<form action="menuController/updateMenu" method="post" class="form form-horizontal" id="form-menu-update">
		<div class="row cl">
			...
			<div class="formControls col-5">
				...
				<input type="text" class="input-text" value="${menu.menuName}" placeholder="" id="menuName" name="menuName" datatype="*2-16" nullmsg="菜单名称不能为空">
			</div>
			<div class="col-4"> </div>
		</div>
		<div class="row cl">
				...
				<div class="formControls col-5">
					<input type="text" class="input-text" value="${menu.menuParentName}" placeholder=""
						id="parentName" name="menuParentName" datatype="*1-8"
						nullmsg="所属菜单不能为空" errormsg="所属菜单的长度必须是1~8位" readonly="readonly">
					
					<input type="hidden" id="parentId" name="menuParentId" value="${menu.menuParentId}">
		...
		<div class="row cl">
			...
			<div class="formControls col-5">
				<textarea id="descContent" name="descContent" cols="" rows="" class="textarea"  placeholder="${menu.descContent}" dragonfly="true" onKeyUp="textarealength(this,150)"></textarea>
...
<script type="text/javascript">
	$(function(){
		...
		/* var menu = new Object();
		menu.id = "${param.id}";
		$.getJSON("menu/getMenuById", menu, function(data){
			debugger
			$("#menuId").val(data.menuId);
			$("#menuName").val(data.menuName);
			$("#parentId").val(data.menuParentId);
			$("#parentName").val(data.menuParentName);
		...
		$("#form-menu-update").Validform(
			{
				...
				debugger
				submitForm("form-menu-update", "menuController/updateMenu");
				return false;
			}
		});
		
		 //处理状态
		 $("#isPublish").val("${menu.isPublish}")
		 //处理类型
